    ...
    if (types.isObjectProperty(node)) {
        if (options.extraKeyNames.includes(node.key.name)) {
            ...
            if (types.isNumericLiteral(nodeVal)) {
                ...
                let flag = true
                // 如果字面量表达式中包含%，则不做转换
                if (types.isTemplateLiteral(nodeVal) && nodeVal.quasis) {
                    for (let i = 0; i < nodeVal.quasis.length; i++) {
                        const element = nodeVal.quasis[i];
                        if (element.value && element.value.raw && element.value.raw.includes('%')) {
                            flag = false
                            break
                        }
                    }
