...
const plugin = declare((api, opts) => {
    ...
    const checkFileIsInclude = (filename, options) => {
    ...
    const convertPxToDp = (node) => {
        if (types.isObjectProperty(node)) {
            if (options.extraKeyNames.includes(node.key.name)) {
                ...
                } else if (types.isIdentifier(node.value)
                    || types.isLogicalExpression(node.value)
                    || types.isTemplateLiteral(node.value)
                    || (types.isCallExpression(node.value) && node.value.callee && node.value.callee.name !== 'pxToDp')) {
                    const { value } = node;
                    const dynamicExpression = types.binaryExpression(
                        '/',
                        types.binaryExpression(
                            ...
                            types.parenthesizedExpression(value)
                        ),
                        types.numericLiteral(options.uiWidth)
                    );
                    node.value = dynamicExpression;
    ...
    return {
        visitor: {
            Program(path, state) {
                let isValid = false;
                isValid = checkFileIsInclude(state.filename, options);
                if (isValid) {
